...
export function ArticleCardsSkeleton() {
  return <div role="status" aria-label="আরও সংবাদ লোড হচ্ছে"><span className="sr-only">আরও সংবাদ লোড হচ্ছে…</span>{[0, 1, 2].map(i => <div key={i} aria-hidden="true" className="flex animate-pulse gap-5 border-b border-slate-200 py-5 dark:border-white/10"><div className="flex-1 space-y-3"><SkeletonBlock className="h-4 w-28" /><SkeletonBlock className="h-6 w-full" /><SkeletonBlock className="h-4 w-4/5" /><SkeletonBlock className="h-4 w-3/4" /></div><SkeletonBlock className="h-24 w-26 shrink-0 sm:h-28 sm:w-44" /></div>)}</div>;
}
