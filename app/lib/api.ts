...
export class ApiError extends Error {
  constructor(public status: number) {
    super("Article service unavailable");
  }
}
...
function normalize<T extends Article | ArticleDetails>(article: T): T {
  if (
    ...
    !article.portal ||
    typeof article.portal.name !== "string" ||
    typeof article.publishedAt !== "string" ||
    typeof article.category !== "string"
  )
    throw new ApiError(502);
  if ("summary" in article && typeof article.summary !== "string")
    throw new ApiError(502);
  if ("details" in article && typeof article.details !== "string")
    throw new ApiError(502);
  ...
  if (
    ...
    data.some(
      (c) =>
        ...
        !/^[a-z0-9-]{1,80}$/.test(c.name) ||
        (c.nameBn !== null && typeof c.nameBn !== "string") ||
        (c.nameEn !== null && typeof c.nameEn !== "string"),
    )
  )
    throw new ApiError(502);
...
export async function getArticles(
  ...
  return {
    ...data,
    items: data.items.map((item: Article) => normalize(item)),
    hasNext: data.hasNext && !!data.nextCursor,
  };
}
export const getArticle = cache(
  async (id: string): Promise<ArticleDetails> =>
    normalize(await request("/api/v1/articles/" + id)),
);
...
export async function getRelatedArticles(
  ...
  const data: unknown = await request(
    "/api/v1/articles/" + id + "/related?limit=6",
    15_000,
  );
  if (!Array.isArray(data)) throw new ApiError(502);
  const seen = new Set<number>([id]);
  return data
    .filter((item): item is RelatedArticle => {
      if (
        !item ||
    ...
    .map((item) => ({
      ...item,
      mainImage: imageUrl(item.mainImage) ?? null,
      portalLogo: imageUrl(item.portalLogo) ?? null,
    }));
}
...
export const getPortals = cache(async (): Promise<Portal[]> => {
  ...
  return data
    ...
    .map((item) => ({
      id: item.id,
      name: item.name,
      nameBn: typeof item.nameBn === "string" ? item.nameBn : null,
      nameEn: typeof item.nameEn === "string" ? item.nameEn : null,
      logo: imageUrl(item.logo) ?? null,
...
export async function getTrending(): Promise<TrendingCluster[]> {
  ...
  const seen = new Set<number>();
  return data
    .filter((item): item is TrendingCluster => {
      if (
        !item ||
...
export async function getBookmarks(
  ...
  if (
    ...
    throw new ApiError(502);
  return {
    items: data.items.map((entry: { article: Article }) =>
      normalize({
        ...entry.article,
